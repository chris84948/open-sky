...
final class GoogleMapController
  ...
  public void onMapReady(GoogleMap googleMap) {
    ...
    setTileOverlayForWeatherForecast();
  }

  private List<TileOverlay> tileOverlayList = new ArrayList<TileOverlay>();
  private int tileOverlayIndex;
  private boolean cancelCurrentRunner;
  private boolean hasAnimated;
  Handler timerHandler = new Handler();
  Runnable timerRunnable = new Runnable() {
    @Override
    public void run() {
        if (cancelCurrentRunner) {
          for (int i = 0; i < tileOverlayList.size(); i++)
            tileOverlayList.get(i).setVisible(false);  
          tileOverlayList.get(tileOverlayList.size() - 1).setVisible(true);
          cancelCurrentRunner = false;
          return;
        }

        try {
          tileOverlayList.get(tileOverlayIndex).setVisible(false);

          tileOverlayIndex++;
          if (tileOverlayIndex == tileOverlayList.size())
            tileOverlayIndex = 0;

          tileOverlayList.get(tileOverlayIndex).setVisible(true);
          timerHandler.postDelayed(this, 400);
        } catch (Exception ex) {
          // do nothing
        }
    }
  ...
  private void setTileOverlayForWeatherForecast() {
    // Create the tiles, and make them visible
    for (int i = -120; i <= 0; i += 10)
      tileOverlayList.add(getTileOverlayOptions(i));

    // Now hide them except for the current one
    for (int i = 0; i < tileOverlayList.size(); i++)
      tileOverlayList.get(i).setVisible(false);
  ...
  private void setTileAnimation(boolean active) {
    if (active) {
      if (!hasAnimated) {
        hasAnimated = true;
        // Spin through the layers quickly the first time (it's a little jerky)
        for (int i = 0; i < tileOverlayList.size(); i++) {
          tileOverlayList.get(i).setVisible(true);
          tileOverlayList.get(i).setVisible(false);
        }
      }
      ...
      cancelCurrentRunner = true;
    }
  }

  private TileOverlay getTileOverlayOptions(int minuteOffset) {
    TileOverlayOptions tileOverlayOptions = new TileOverlayOptions();
    tileOverlayOptions.transparency(0);
    tileOverlayOptions.fadeIn(false);

    TileOverlay tileOverlay = googleMap.addTileOverlay(tileOverlayOptions.tileProvider(getTileOverlay(minuteOffset)));
    tileOverlay.setVisible(false);
    return tileOverlay;
  }

  private UrlTileProvider getTileOverlay(int minOffset) {
    final int offset = minOffset;
    return new UrlTileProvider(256, 256) {
      ...
      public URL getTileUrl(int x, int y, int zoom) {
        ...
        cal.set(Calendar.MINUTE, minute - minute % 10); // Get neared 10 mins, i.e. 10:10, 10:20, 10:30
        cal.set(Calendar.SECOND, 0);
        cal.set(Calendar.MILLISECOND, 0);
        cal.add(Calendar.MINUTE, offset);

        String url = String.format("https://tilecache.rainviewer.com/v2/radar/%d/256/%d/%d/%d/8/0_0.png", cal.getTimeInMillis() / 1000, zoom, x, y);
  ...
  public void onMethodCall(MethodCall call, MethodChannel.Result result) {
    switch (call.method) {
      ...
      case "tileAnimation#set":
        {
          setTileAnimation((boolean) call.arguments);
          result.success(null);
          break;
        }
      default:
